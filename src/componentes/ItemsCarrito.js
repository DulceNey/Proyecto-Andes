...
const ItemsCarrito = ({data,deleteFromCart,incrementarContador,decrementarContador}) => {
  //const{products, cart}=shoppingInitialState;
    const {id,nombre,precio,cantidad}=data

  return (
  <>
    <div className='w-full flex flex-row justify-between gap-12 md:justify-between items-center pl-3 text-slate-900 bg-slate-500 md:w-full'>
      <div className=' tracking-wide p-3 '>
        ...
        type="button" className="text-slate-900 bg-slate-400  focus:ring-gray-300 font-medium rounded-lg text-sm px-2.5 py-0 mr-2 mb-2">+</button>
        ...
        type="button" className="text-slate-900 bg-slate-400  focus:ring-gray-300 font-medium rounded-lg text-sm px-3 py-0 mr-2 mb-2">-</button>
        </div>
      </div>

    <div className=" w-fit mr-3 pr-1 ml-0 md:mr-3">
    
        <button onClick={()=>deleteFromCart(id)} className="pl-5 mr-3 md:pl-8" >
